...
class Solution:
    ...
    def buildArray(self, nums: List[int]) -> List[int]:
        ...
        index i, and we need only the original value at index n to store it as a quotient
        at index i. By following the same logic, the original value at index n is obtained
        by extracting only the remainder from nums[n].
